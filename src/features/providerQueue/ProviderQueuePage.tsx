...
export function ProviderQueuePage(){
  ...
  return <main className="operations-shell">
    ...
    <section className="provider-list" aria-busy={loading}>{loading?<OpticalCard><p aria-live="polite">{c.loading}</p></OpticalCard>:items.length===0?<OpticalCard>{c.empty}</OpticalCard>:items.map(i=><OpticalCard key={i.id} className="provider-row"><div><OpticalStatus tone={i.priority==='urgent'?'warning':i.priority==='review_recommended'?'signal':'neutral'}>{i.priority==='urgent'?c.urgent:i.priority==='review_recommended'?c.review:c.routine}</OpticalStatus><h2>{i.patientName}</h2><p>{c.sla}: <strong>{new Date(i.respondBy).toLocaleString(language)}</strong></p>{i.appointmentAt&&<p>{c.scheduledFor}: <strong>{new Date(i.appointmentAt).toLocaleString(language)}</strong></p>}</div><div><OpticalStatus>{c[i.status as keyof typeof c]??i.status}</OpticalStatus><OpticalButton onClick={()=>void open(i.id)}>{c.open}</OpticalButton></div></OpticalCard>)}</section>
...
function ProviderDrawer({selected,c,language,pending,close,act,openDocument,organizationId}:{selected:QueueDetail;c:(typeof providerCopy)[keyof typeof providerCopy];language:'en'|'ru';pending:ActionKind|null;close:()=>void;act:(kind:ActionKind,fn:(key:string)=>Promise<void>)=>Promise<void>;openDocument:(path:string)=>void;organizationId:string}){
  ...
  return <div className="operations-modal" role="dialog" aria-modal="true"><OpticalCard className="operations-drawer"><button className="operations-close" onClick={close} aria-label={c.close}>×</button><OpticalEyebrow>{selected.patientName}</OpticalEyebrow><h2>{selected.priority==='urgent'?c.urgent:selected.priority==='review_recommended'?c.review:c.routine}</h2><OpticalStatus>{c[selected.status as keyof typeof c]??selected.status}</OpticalStatus>{selected.appointment&&<p>{c.scheduledFor}: <strong>{new Date(selected.appointment.scheduledAt).toLocaleString(language)}</strong></p>}

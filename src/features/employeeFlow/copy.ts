...
const en = {
  ...
  referral: {
    ...
    status: 'Status', statusValue: 'Awaiting clinic selection', queued: 'Sent to clinical partner', appointment_booked: 'Appointment booked', urgent_escalated: 'Urgently escalated', examination_completed: 'Examination completed', outcome_confirmed: 'Care result confirmed', deadline: 'Complete by', scheduledFor: 'Scheduled for', next: 'What happens next', steps: ['Choose an approved clinical partner', 'Give explicit consent to share the referral', 'Book an examination'], chooseProvider: 'Clinical partner', consentNotice: 'By continuing, you consent to share this referral with the selected clinical partner. You can revoke access in Profile.', shareAndSend: 'Consent and send referral', sending: 'Sending…', assignmentSuccess: 'Referral sent securely.', assignmentFailed: 'Could not send the referral. Try again.',
...
const ru = {
  ...
  referral: {
    ...
    status: 'Статус', statusValue: 'Ожидает выбора клиники', queued: 'Передано клиническому партнёру', appointment_booked: 'Запись создана', urgent_escalated: 'Срочно эскалировано', examination_completed: 'Обследование завершено', outcome_confirmed: 'Результат помощи подтверждён', deadline: 'Завершить до', scheduledFor: 'Запись на', next: 'Что будет дальше', steps: ['Выбрать проверенного клинического партнёра', 'Явно согласиться на передачу направления', 'Записаться на обследование'], chooseProvider: 'Клинический партнёр', consentNotice: 'Продолжая, вы разрешаете передать это направление выбранному клиническому партнёру. Доступ можно отозвать в профиле.', shareAndSend: 'Разрешить и отправить направление', sending: 'Отправляем…', assignmentSuccess: 'Направление безопасно передано.', assignmentFailed: 'Не удалось передать направление. Повторите попытку.',

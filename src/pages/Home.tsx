  ...
  return (
    <div className="overflow-hidden bg-[#f7f1e8]">
      <section className="relative px-6 py-14 md:py-20">
        <div className="absolute inset-0 bg-[linear-gradient(135deg,#f7f1e8_0%,#fffaf2_54%,#edf3ef_100%)]" />
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-[1fr_0.95fr]">
          <div>
            ...
            <h1 className="max-w-3xl text-4xl font-black leading-[1.04] text-slate-950 sm:text-5xl md:text-6xl">
            ...
            <div className="mt-8 flex flex-col gap-4 sm:flex-row">
              <button
                ...
                className="group rounded-full bg-slate-950 px-8 py-4 text-sm font-bold uppercase tracking-[0.14em] text-white transition hover:-translate-y-0.5 hover:bg-[#315c56]"
              >
                Начать подбор <ArrowRight className="ml-2 inline transition group-hover:translate-x-1" size={18} />
              </button>
              <button
                onClick={() => onNavigate('products')}
                className="rounded-full border border-slate-950/20 bg-white/70 px-8 py-4 text-sm font-bold uppercase tracking-[0.14em] text-slate-950 backdrop-blur transition hover:bg-white"
              >
                Смотреть каталог
              </button>
            </div>
            <div className="mt-8 grid gap-3 text-sm font-bold text-slate-700 sm:grid-cols-3">
              {['Фото остается в браузере', 'Face-fit score', 'Оптики после подбора'].map((label) => (
                <div key={label} className="rounded-2xl bg-white/70 px-4 py-3 ring-1 ring-slate-900/10">
                  <CheckCircle2 className="mb-2 text-[#315c56]" size={18} />
                  {label}
                </div>
              ))}
            </div>
          ...
          <div className="relative">
            <div className="absolute -left-5 top-7 h-full w-full rotate-[-4deg] rounded-[1.75rem] bg-[#315c56]" />
            <div className="relative overflow-hidden rounded-[1.75rem] bg-white p-4 shadow-2xl shadow-slate-900/20">
              ...
              <div className="absolute bottom-8 left-8 right-8 rounded-[1.25rem] bg-white/88 p-5 backdrop-blur-xl">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#315c56]">Путь пользователя</p>

...
export function Products({ onNavigate, fittingCart, onToggleFitting }: ProductsProps) {
  ...
  return (
    <div className="min-h-screen bg-vilu-paper">
      <section className="border-b border-vilu-line bg-vilu-cream px-6 py-14">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end">
          <div>
            ...
            <h1 className="mt-4 max-w-4xl text-4xl font-black leading-[1.06] text-vilu-ink sm:text-5xl md:text-6xl">Выберите оправы, проверьте посадку и идите в салон с коротким списком.</h1>
          ...
          <div className="rounded-[2rem] bg-white p-5 shadow-xl shadow-slate-900/10 ring-1 ring-vilu-line">
            ...
            <div className="mt-5 grid gap-2">
              {flowSteps.map(({ label, value, icon: Icon }) => (
                <div key={label} className="flex items-center justify-between rounded-2xl bg-vilu-paper px-4 py-3">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 lg:grid-cols-[280px_1fr]">
          ...
          <div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-vilu-line">
            ...
            <div className="space-y-7">
                ...
                <div className="flex flex-wrap gap-2 lg:flex-col">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-left text-sm font-bold transition ${categoryFilter === category.id ? 'bg-vilu-ink text-white' : 'bg-vilu-paper text-slate-600 hover:bg-vilu-mist'}`}
                    >
                      {category.name}
                    </button>
                  ))}
              ...
              <div>
                ...
                <div className="grid gap-2">
                  {[
                    ...
                    <button key={brand.id} onClick={() => setBrandFilter(brand.id)} className={`rounded-2xl px-4 py-3 text-left text-sm font-bold transition ${brandFilter === brand.id ? 'bg-vilu-green text-white' : 'bg-vilu-paper text-slate-600 hover:bg-vilu-mist'}`}>{brand.name}</button>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className={`flex w-full items-center justify-between rounded-3xl p-4 text-left transition ${onlyFitting ? 'bg-vilu-amber text-vilu-ink' : 'bg-vilu-ink text-white'}`}
              >
        ...
        <section>
          <div className="mb-8 flex items-center justify-between border-b border-vilu-line pb-4">
            ...
            <button onClick={() => { setCategoryFilter('all'); setBrandFilter('all'); setOnlyFitting(false); }} className="text-sm font-bold text-vilu-green">Сбросить</button>
          </div>
          ...
          <div className="grid gap-x-7 gap-y-10 sm:grid-cols-2 xl:grid-cols-3">
            {filteredProducts.map((product) => {
              ...
              return (
                <article key={product.id} className="group rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-vilu-line transition hover:-translate-y-1 hover:shadow-xl">
                  <button onClick={() => onNavigate('product', product.id)} className="block w-full text-left">
                    ...
                    <div className="px-2 pt-5">
                      ...
                      <div className="mt-2 flex items-start justify-between gap-4">
                        <h3 className="text-2xl font-black tracking-tight text-vilu-ink">{product.name}</h3>
                  ...
                  <div className="mt-5 grid gap-2 sm:grid-cols-2">
                    <button onClick={() => onNavigate('product', product.id)} className="rounded-full border border-vilu-line px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-vilu-ink transition hover:bg-vilu-ink hover:text-white">Подробнее</button>
                    {canFit ? (
                      <button onClick={() => onToggleFitting(product.id)} className={`rounded-full px-4 py-3 text-xs font-black uppercase tracking-[0.16em] transition ${inFitting ? 'bg-vilu-green text-white' : 'bg-vilu-amber text-vilu-ink hover:bg-vilu-amber/90'}`}>{inFitting ? 'В примерке' : 'Примерить'}</button>
                    ) : (

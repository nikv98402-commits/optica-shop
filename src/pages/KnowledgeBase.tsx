...
export const knowledgePages: KnowledgePage[] = [
  ...
  },
  {
    ...
    shortAnswer: 'В MVP-версии личный кабинет ViLu работает в demo/local mode: данные анкеты, рецепта и тренировок сохраняются в браузере пользователя и не отправляются на сервер ViLu.',
    ...
    limits: [
      ...
      'Передача реальных персональных данных в облако будет включаться только после отдельной юридической и технической готовности.',
    ],
    example: 'Если пользователь изменил имя и рецепт в кабинете, эти данные остаются в браузере. В Метрику отправляется только событие profile_saved_local без значений полей.',

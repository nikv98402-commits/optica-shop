...
const textTranslations: Record<string, string> = {
  ...
  'Приоритетная подготовка визита': 'Priority visit preparation',
  'Консультант заранее получает ваш короткий список, готовит похожие оправы и экономит время визита.': 'A consultant gets your shortlist in advance, prepares similar frames, and saves visit time.',
  'тест': 'test',
  'Проверить готовность платить': 'Check willingness to pay',
  'Сначала сохраните хотя бы одну оправу в подбор.': 'Save at least one frame first.',
  'Оплата скоро': 'Payment coming soon',
  'Это фейк-дверь для проверки спроса: платежная интеграция пока не подключена, деньги не списываются.': 'This is a fake door to test demand: payment integration is not connected yet, and no money is charged.',
  'Цена': 'Price',
  'Данные': 'Data',
  'без фото и рецепта': 'no photo or prescription',
  'Что входит в сервис': 'What is included',
  '1. Проверка короткого списка 2-3 оправ перед визитом.': '1. Review of a 2-3 frame shortlist before the visit.',
  '2. Подготовка похожих моделей консультантом.': '2. Similar models prepared by a consultant.',
  '3. Чеклист вопросов по посадке, мосту и ширине оправы.': '3. Checklist for fit, bridge, and frame width questions.',
  'Платежи пока не подключены. Мы зафиксировали интерес к сервису и не списали деньги.': 'Payments are not connected yet. We recorded interest in the service and did not charge money.',
  'Оплатить 429 ₽': 'Pay 429 ₽',
  'Зафиксировать интерес 429 ₽': 'Register interest 429 ₽',
  'Тестовый контур оплаты': 'Test payment contour',
  'Проверяем сценарий оплаты без реального списания. Банковские данные не запрашиваются и не сохраняются.': 'We are testing the payment flow without a real charge. Bank card data is neither requested nor stored.',
  'Создаем безопасный тестовый платеж. Деньги не списываются.': 'Creating a safe test payment. No money is charged.',
  'Перейти к тесту оплаты 429 ₽': 'Open payment test 429 ₽',
  'Создаем тест...': 'Creating test...',
  'Фиксируем...': 'Registering...',
  'Фиксируем интерес. Деньги не списываются.': 'Registering interest. No money is charged.',
  'Платежи пока не подключены. Мы зафиксировали интерес локально и не списали деньги.': 'Payments are not connected yet. We registered interest locally and did not charge money.',
  'Интерес к сервису сохранен. Платежный провайдер пока не подключен, деньги не списаны.': 'Service interest saved. The payment provider is not connected yet, and no money was charged.',
  'Не сейчас': 'Not now',
  ...
  'Готовим подбор. Фото, рецепт и точные координаты не отправляются.': 'Preparing the selection. Photo, prescription, and exact coordinates are not sent.',
  'Подбор сохранен для подготовки визита. Фото, рецепт и точные координаты не отправлены.': 'Selection saved for visit preparation. Photo, prescription, and exact coordinates were not sent.',
  ...
  'Подбор подготовлен локально, но браузер запретил копирование. Данные не отправлены на сервер.': 'Selection prepared locally, but the browser blocked copying. Data was not sent to the server.',
  ...
  'Подбор готов локально, но браузер запретил копирование. Контакт не отправлен.': 'Selection is ready locally, but the browser blocked copying. Contact was not sent.',
  'Браузер запретил копирование. Подбор остался только на этом устройстве.': 'The browser blocked copying. The selection stayed on this device only.',

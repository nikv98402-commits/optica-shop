...
const frameColors: Record<Product['category'], string> = {
  eyeglasses: 'border-vilu-ink',
  sunglasses: 'border-[#5a3926] bg-[#2d221b]/30',
  contact_lenses: 'border-vilu-ink/20',
};
...
export function VirtualTryOn({ product, compact = false }: VirtualTryOnProps) {
  ...
  return (
    <section className={`rounded-[2rem] bg-vilu-ink p-5 text-vilu-paper ${compact ? '' : 'mt-8'}`}>
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--vilu-lime)]">Виртуальная примерка</p>
          <h2 className="mt-2 text-2xl font-black tracking-tight">{product.name}</h2>
        </div>
        <div className="rounded-2xl bg-vilu-paper/10 p-3 text-[var(--vilu-lime)]">
          <Camera size={22} />
        </div>
      </div>
      ...
      <div className="relative mx-auto flex aspect-[4/3] max-w-xl items-center justify-center overflow-hidden rounded-[1.5rem] bg-[#f7f1e8]">
        <div className={`relative h-[70%] w-[46%] min-w-44 rounded-[48%] bg-[#d7a77d] shadow-inner ${faceShape === 'round' ? 'w-[50%]' : ''} ${faceShape === 'angular' ? 'rounded-[38%]' : ''}`}>
          <div className="absolute left-[27%] top-[36%] h-3 w-3 rounded-full bg-vilu-ink" />
          <div className="absolute right-[27%] top-[36%] h-3 w-3 rounded-full bg-vilu-ink" />
          ...
          <div className="absolute left-1/2 top-[35%] flex -translate-x-1/2 items-center" style={{ width: `${fitSize}%` }}>
            <div className={`h-16 flex-1 border-[7px] ${frameClass} ${lensRadius} bg-vilu-paper/10 shadow-lg`} />
            <div className={`${bridgeWidth} h-2 rounded-full bg-vilu-ink`} />
            <div className={`h-16 flex-1 border-[7px] ${frameClass} ${lensRadius} bg-vilu-paper/10 shadow-lg`} />
          </div>
        </div>

        <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-vilu-card/85 p-3 text-sm font-bold text-vilu-ink/72 backdrop-blur">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_180px]">
        <div>
          <p className="mb-3 text-xs font-black uppercase tracking-[0.18em] text-vilu-paper/45">Форма лица</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button
                ...
                className={`rounded-2xl px-3 py-3 text-sm font-black transition ${faceShape === id ? 'bg-[var(--vilu-lime)] text-vilu-ink' : 'bg-vilu-paper/10 text-vilu-paper hover:bg-vilu-paper/15'}`}
              >
                {label}
              </button>
            ))}
        ...
        <label>
          <span className="mb-3 block text-xs font-black uppercase tracking-[0.18em] text-vilu-paper/45">Размер</span>
          <input
            type="range"
            min="62"
            max="84"
            ...
            className="w-full accent-[var(--vilu-lime)]"
          />
          <button
            type="button"
            onClick={() => { setFaceShape('oval'); setFitSize(72); }}
            className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-vilu-paper/10 px-3 py-3 text-xs font-black uppercase tracking-[0.14em] text-vilu-paper transition hover:bg-vilu-paper/15"
          >
      ...
      <div className="mt-5 flex gap-3 rounded-2xl bg-vilu-paper/10 p-4 text-sm leading-6 text-vilu-paper/70">
        <Sparkles className="mt-1 shrink-0 text-[var(--vilu-lime)]" size={18} />

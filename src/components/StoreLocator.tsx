                ...
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div>
                    ...
                    <div className="mt-4 space-y-3 text-sm text-slate-600">
                      ...
                      <p className="flex gap-3"><Phone className="shrink-0 text-[#315c56]" size={18} /> {store.phone ?? 'Телефон уточняется'}</p>
                      <p className="flex gap-3"><Clock className="shrink-0 text-[#315c56]" size={18} /> {storeHoursLabel(store)}</p>
                    </div>
                  </div>
                  <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-green-700">{storeStatusLabel(store)}</span>
                </div>
              </article>
            ))}
            {filteredStores.length === 0 && (
              <div className="rounded-3xl bg-white p-6 text-sm font-bold text-slate-500 ring-1 ring-slate-900/5">
